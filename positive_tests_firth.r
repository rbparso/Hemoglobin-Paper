...
for (i in 1:nrow(data_no_null)) {
   data_no_null$num_of_symptoms[i] = sum(data_no_null[i,c(21:37)], na.rm=TRUE)
   if (data_no_null$num_of_symptoms[i] == 0) {
      data_no_null$symptom_category[i] = "0"
   } else if (data_no_null$num_of_symptoms[i] == 1 |
              data_no_null$num_of_symptoms[i] == 2) {
      data_no_null$symptom_category[i] = "1-2"
   } else if (data_no_null$num_of_symptoms[i] == 3 |
              data_no_null$num_of_symptoms[i] == 4) {
      data_no_null$symptom_category[i] = "3-4"
   } else {
      data_no_null$symptom_category[i] = "5+"
   }
}
...
names(data_no_null)[86] = "Numeric_COVID_Test_Result"
names(data_no_null)[88] = "Symptoms_Number"
...
names(data_no_unsure)[86] = "Numeric_COVID_Test_Result"
names(data_no_unsure)[88] = "Symptoms_Number"

result_sc = logistf(Numeric_COVID_Test_Result ~ Sickle_Cell_Presence,
    family=binomial, data=data_no_unsure)
summary(result_sc)
result_sym = logistf(Numeric_COVID_Test_Result ~ Symptoms_Number,
    family=binomial, data=data_no_unsure)
summary(result_sym)
...
names = c("Intercept","1-2 Symptoms","3-4 Symptoms", "5+ Symptoms")
to_table = matrix(c(names, odds, conf.int, pval), ncol=4)
colnames(to_table) = c("Variable", "Odds", "95% CI", "p-value")
table = as.data.frame(to_table[c(-1),])
table %>%
